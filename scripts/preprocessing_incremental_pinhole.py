...
def fix_cameras_bin_from_db(db_path: Path, sparse_dir: Path):
    ...
    with open(sparse_dir / "cameras.bin", "wb") as f:
        f.write(struct.pack("<Q", len(cameras)))  # num_cameras
        for row in cameras:
            ...
            print(f"[preprocessing] Fixed camera {camera_id}: {width}x{height}, fx={fx:.2f}, fy={fy:.2f}, cx={cx:.2f}, cy={cy:.2f}")
...
def process_t0(t0_dir: Path, vocab_tree_path: str):
    ...
    # IMPORTANT: Do NOT call fix_cameras_bin_from_db here!
    # t0 database may have NULL params, which would result in wrong values (e.g., fx=1152)
    # Keep the mapper's original cameras.bin output as the model was trained with those values
    print(f"\n[PINHOLE] Skipping image_undistorter (PINHOLE has no distortion)")
    print(f"[t0] Keeping original cameras.bin from mapper (model was trained with these values)")
...
def process_tn(
    ...
    # --Mapper.fix_existing_images=1 prevents BA from modifying existing t0 camera poses
    run_colmap([
    ...
    # Note: Skipping bundle_adjuster to preserve t0 camera poses exactly
    # image_registrator with fix_existing_images already estimates t1 poses

    # For PINHOLE: skip image_undistorter (no distortion to remove)
    # IMPORTANT: Use t0 cameras.bin to ensure same intrinsics as training
    # DO NOT call fix_cameras_bin_from_db here as it would write wrong values
    # (t1 database may have different params than what the model was trained with)
    print(f"\n[PINHOLE] Skipping image_undistorter (PINHOLE has no distortion)")
    t0_cameras_bin = t0_dir / "sparse" / "0" / "cameras.bin"
    if t0_cameras_bin.exists():
        shutil.copy2(t0_cameras_bin, ws_sparse / "cameras.bin")
        print(f"[{tn_dir.name}] Copied cameras.bin from t0 to preserve training intrinsics")
...
def main():
    ...
    parser.add_argument(
        "--skip-t0",
        action="store_true",
        help="Skip t0 processing and only run incremental registration for t1+.",
    )
    ...
    # -------------------------------------------------------------------------
    # Process t0 (baseline) - skip if --skip-t0
    # -------------------------------------------------------------------------
    t0_dir = t_dirs[0]
    if not args.skip_t0:
        process_t0(t0_dir, vocab_tree)
    else:
        print(f"\n[SKIP] t0 processing skipped (--skip-t0)")
