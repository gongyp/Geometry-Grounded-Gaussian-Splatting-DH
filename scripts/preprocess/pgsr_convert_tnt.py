...
def init_colmap(args):
    ...
    # Use multiple threads for CPU acceleration
    num_threads = 32
    ...
    for scene in scene_list:
        ...
        # extract features with multi-threaded CPU
        print("Step 1: Feature extraction...")
        ret = os.system(f"colmap feature_extractor --database_path {scene_path}/database.db \
        ...
        if ret != 0:
            print(f"Warning: Feature extraction returned {ret}")

        # match features with multi-threaded CPU
        print("Step 2: Feature matching...")
        ret = os.system(f"colmap sequential_matcher \
                --database_path {scene_path}/database.db \
                --SiftMatching.use_gpu=false \
                --SiftMatching.num_threads={num_threads}")
        if ret != 0:
            print(f"Warning: Feature matching returned {ret}")

        # read poses
        print("Step 3: Reading COLMAP poses...")
        ...
        # bundle adjustment
        print("Step 5: Point triangulator...")
        ret = os.system(f"colmap point_triangulator \
                --database_path {scene_path}/database.db \
                --image_path {scene_path}/images_raw \
        ...
        if ret != 0:
            print(f"Warning: Point triangulator returned {ret}")

        print("Step 6: Bundle adjustment...")
        ret = os.system(f"colmap bundle_adjuster \
                --input_path {scene_path}/sparse \
                --output_path {scene_path}/sparse \
                --BundleAdjustment.refine_extrinsics=false")
        if ret != 0:
            print(f"Warning: Bundle adjuster returned {ret}")

        # undistortion
        print("Step 7: Image undistortion...")
        ret = os.system(f"colmap image_undistorter \
            --image_path {scene_path}/images_raw \
            --input_path {scene_path}/sparse \
            --output_path {scene_path} \
            --output_type COLMAP")
        if ret != 0:
            print(f"Warning: Image undistorter returned {ret}")

        # read for bounding information
        print("Step 8: Computing bounding info...")
        ...
        # colmap to json
        print("Step 9: Exporting to JSON...")
        cameras, images, points3D = read_model(os.path.join(args.tnt_path, scene, 'sparse'), ext='.txt')
        export_to_json(cameras, images, bounding_box, list(center), radius, os.path.join(scene_path, 'transforms.json'))
        print(f'Writing data to json file: {scene_path}/transforms.json')
