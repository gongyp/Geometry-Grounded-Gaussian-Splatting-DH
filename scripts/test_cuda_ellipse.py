#!/usr/bin/env python
"""Test ellipse_projection_cuda.py"""
...
# Import the CUDA ellipse projection directly (bypass incremental/__init__.py)
spec = importlib.util.spec_from_file_location(
    "ellipse_projection_cuda",
    "/data/Geometry-Grounded-Gaussian-Splatting/incremental/lifter/ellipse_projection_cuda.py"
)
ellipse_cuda_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(ellipse_cuda_module)
fast_pixel2gaussians = ellipse_cuda_module.fast_pixel2gaussians
...
def main():
    ...
    # Test with 50000 Gaussians first
    N = 50000
    ...
    t0 = time.time()
    pixel_gaus = fast_pixel2gaussians(
    ...
    # pixel_gaus is [H] x [W] list of gaussian index lists
    # Count mappings
    total_mappings = 0
    pixels_with_gaussians = 0
    for r in range(H):
        for c in range(W):
            gaussians = pixel_gaus[r][c]
            if len(gaussians) > 0:
                pixels_with_gaussians += 1
                total_mappings += len(gaussians)
    print(f"Pixels covered: {pixels_with_gaussians}/{H*W}")
